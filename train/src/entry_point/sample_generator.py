...
def generate_and_tokenize_prompt_qwen(
    model_max_length: int,
    tokenizer: PreTrainedTokenizer,
    data_point: Dict[str, Any],
    fix_length=False,
...
def generate_and_tokenize_prompt_qwen_gis(
    model_max_length: int,
    tokenizer: PreTrainedTokenizer,
    data_point: Dict[str, Any],
    fix_length=False,
    ...
    input_ids = []
    labels = []
    sentence_value1 = "<|im_start|>Human:\n温度：" + data_point['temperature'] + '<|im_end|>' + '\n' + "<|im_start|>Assistant:\n"
    sentence_value2 = "设备状态：" + data_point['state'] + '\n判断依据：' + data_point['reason'] + '\n运维建议：' + data_point['maintenance'] + '<|im_end|>'
    sentence_ids1 =  tokenizer.encode(sentence_value1, add_special_tokens=False)
    sentence_ids2 =  tokenizer.encode(sentence_value2, add_special_tokens=False)
    input_ids = sentence_ids1 + sentence_ids2
    labels = [IGNORE_INDEX] * len(sentence_ids1) + copy.deepcopy(sentence_ids2)
